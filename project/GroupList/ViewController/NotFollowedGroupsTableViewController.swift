...
private extension NotFollowedGroupsTableViewController {
    ...
    //    func groups(_ name: String, _ iconName: String){
    //        let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
    //        let group = FollowedGroups(entity: FollowedGroups.entity(), insertInto: context)
    //        let groupNameKey = "name"
    //        let groupIconKey = "iconName"
    //        group.setValue(name, forKey: groupNameKey)
    //        group.setValue(iconName, forKey: groupIconKey)
    //        do {
    //            try context.save()
